...
if st.button("💾 Guardar outfit manual"):
    if not o_name or not item_ids: st.error("Pon nombre y al menos una prenda.")
    else:
        db_outfit_insert({
...
def auto_build(df, tmin, tmax, eleg):
    ...
    if tmax>=25 and not shoes[shoes["categoria"]=="Sandalias"].empty: spool = shoes[shoes["categoria"]=="Sandalias"]
    elif tmax<=12 and not shoes[shoes["categoria"]=="Botas"].empty: spool = shoes[shoes["categoria"]=="Botas"]
    else: spool = shoes
