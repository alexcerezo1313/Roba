...
def swatch(hex_code, label=None):
    ...
    st.markdown(
        ...
        f"<code>{hex_code}</code>{lab}</div>", unsafe_allow_html=True
    )

# ── Colores: HSV helpers, cuantización y reglas ───────────────────────────────
def rgb_to_hsv_vec(rgb_arr_uint8: np.ndarray) -> np.ndarray:
    ...
    h = np.zeros_like(mx)
    mask_r = (mx==r) & (diff!=0); mask_g = (mx==g) & (diff!=0); mask_b = (mx==b) & (diff!=0)
...
def hex_from_rgb(rgb): r,g,b = [int(x) for x in rgb[:3]]; return f"#{r:02X}{g:02X}{b:02X}"
...
def quantize_colors(arr_rgb_uint8: np.ndarray, k=6):
    ...
    pal = q.getpalette()[:k*3]; counts = q.getcolors() or []
    res = []
...
def auto_colors_from_image(image: Image.Image):
    ...
    w0,h0 = image.size; scale = min(640 / max(w0, h0), 1.0)
    if scale < 1.0:
    ...
    pal = quantize_colors(selected, k=k_eff); total = sum(c for c,_ in pal) if pal else 1
    c1 = pal[0][1] if pal else None; c2 = None
    def dist(a,b):
        a = rgb_to_hsv_vec(np.array([a], dtype=np.uint8))[0]
        b = rgb_to_hsv_vec(np.array([b], dtype=np.uint8))[0]
        dh = min(abs(a[0]-b[0]), 1-abs(a[0]-b[0])) * 2.0; ds = abs(a[1]-b[1]); dv = abs(a[2]-b[2])
...
def hex_family(hex_code):
    ...
    if max(r,g,b)-min(r,g,b) < 18:
        ...
        return "gray"
    hsv = rgb_to_hsv_vec(np.array([[r,g,b]], dtype=np.uint8))[0]; hue = hsv[0]*360
...
with st.sidebar:
    st.header("🔑 Cuenta")
    mode = st.radio("Acceso", ["Entrar", "Registrarme"], horizontal=True)
    ...
    if mode == "Entrar":
        if cA.button("Entrar"):
            try:
                ...
                st.toast("Sesión iniciada ✅"); time.sleep(0.4); st.rerun()
    ...
    if cB.button("Salir"): sb.auth.sign_out(); st.info("Sesión cerrada"); time.sleep(0.3); st.rerun()
...
# ──────────────────────────────────────────────────────────────────────────────
# Añadir prenda (3 métodos color, autos con 2 fases)
# ──────────────────────────────────────────────────────────────────────────────
st.title("👕 Armario Digital (Cloud)")

st.subheader("➕ Añadir prenda")
c0, c1, c2, c3 = st.columns([1.2,1,1,1])
with c0: nombre_prenda = st.text_input("Nombre de la prenda", placeholder="Ej. Sudadera azul marino")
with c1: categoria = st.selectbox("Categoría", CATEGORIAS)
with c2: tipo = st.selectbox("Tipo", TIPOS)
with c3: estilo = st.selectbox("Estilo", ESTILOS)

c4, c5 = st.columns([1,1])
with c4: metodo = st.radio("Método de color", ["Paleta","Hex (picker)","Automático desde imagen"], horizontal=True)
with c5: hay_sec = st.checkbox("¿Tiene color secundario?")

ic1, ic2 = st.columns([1,2])
with ic1:
    foto = st.file_uploader("Foto (obligatoria en 'Automático')", type=["png","jpg","jpeg"])
    if foto: st.image(foto, caption="Vista previa", use_container_width=True)
with ic2:
...
if metodo=="Paleta":
    ...
    with p1:
        ...
        color1_hex = DEFAULT_PALETTE[c1name]; swatch(color1_hex,"principal")
    if hay_sec:
        with p2:
            c2name = st.selectbox("Secundario (paleta)", list(DEFAULT_PALETTE.keys()))
            color2_hex = DEFAULT_PALETTE[c2name]; swatch(color2_hex,"secundario")
    n1,n2 = st.columns(2)
    with n1: color1_name = st.text_input("Nombre color principal (opcional)")
    if hay_sec: 
        with n2: color2_name = st.text_input("Nombre color secundario (opcional)")

elif metodo=="Hex (picker)":
    p1,p2 = st.columns(2)
    with p1: color1_hex = st.color_picker("Principal (hex)", "#3366cc"); swatch(color1_hex,"principal")
    if hay_sec:
        with p2: color2_hex = st.color_picker("Secundario (hex)", "#bbbbbb"); swatch(color2_hex,"secundario")
    n1,n2 = st.columns(2)
    with n1: color1_name = st.text_input("Nombre color principal (opcional)")
    if hay_sec: 
        with n2: color2_name = st.text_input("Nombre color secundario (opcional)")
    ...
    if "auto_state" not in st.session_state:
        st.session_state["auto_state"] = {"ready": False, "c1":"", "c2":"", "n1":"", "n2":""}
    if st.button("🔍 Detectar colores"):
        if not foto: st.error("Sube la foto para detectar los colores.")
        else:
            img = Image.open(io.BytesIO(foto.getvalue())).convert("RGB")
            c1,c2 = auto_colors_from_image(img)
            st.session_state["auto_state"] = {"ready": True, "c1": c1 or "", "c2": c2 or "", "n1":"", "n2":""}
    ...
    if A["ready"]:
        ...
        if color1_hex and color2_hex and color1_hex.lower()!=color2_hex.lower():
            a,b=st.columns(2)
            with a: swatch(color1_hex,"principal (auto)")
            with b: swatch(color2_hex,"secundario (auto)")
            ok = st.radio("¿Correcto?",["Sí","No"], horizontal=True, key="okboth")
            if ok=="No":
                x,y=st.columns(2)
                with x: color1_hex = st.color_picker("Principal (ajuste)", color1_hex)
                with y: color2_hex = st.color_picker("Secundario (ajuste)", color2_hex)
        elif color1_hex:
            ...
            if hay_sec:
                ...
                if swap:
                    ...
                    color2_hex = st.color_picker("Secundario (manual hex)", "#bbbbbb"); swatch(color2_hex,"secundario (manual)")
        else:
            st.info("No se detectaron colores. Define por hex:")
            x,y = st.columns(2)
            with x: color1_hex = st.color_picker("Principal (hex)", "#3366cc")
            if hay_sec: 
                with y: color2_hex = st.color_picker("Secundario (hex)", "#bbbbbb")

        a,b=st.columns(2)
        with a: color1_name = st.text_input("Nombre color principal (opcional)", key="n1auto")
        if hay_sec: 
            with b: color2_name = st.text_input("Nombre color secundario (opcional)", key="n2auto")

# Validaciones + Guardado (Storage + DB)
errs=[]
...
if st.button("💾 Guardar prenda", type="primary"):
    if errs:
        ...
        # Subir foto si hay (profesional: Storage con signed URL)
        photo_path = upload_image(user.id, foto) if foto else ""
        db_item_insert({
            "nombre": nombre_prenda, "categoria": categoria, "tipo": tipo, "estilo": estilo,
            "color1_hex": color1_hex or "", "color1_name": color1_name or "",
            "color2_hex": (color2_hex or "") if hay_sec else "", "color2_name": (color2_name or "") if hay_sec else "",
            "photo_path": photo_path
        })
        st.success("Prenda guardada en la nube ✅")
        if "auto_state" in st.session_state: st.session_state["auto_state"]["ready"]=False
...
if items_df.empty:
    ...
    with f1: fcat = st.selectbox("Categoría", ["Todos"]+sorted(items_df["categoria"].dropna().unique().tolist()))
    with f2: ftip = st.selectbox("Tipo", ["Todos"]+sorted(items_df["tipo"].dropna().unique().tolist()))
    with f3: fest = st.selectbox("Estilo", ["Todos"]+ESTILOS)

    df = items_df.copy()
    if fcat!="Todos": df = df[df["categoria"]==fcat]
    if ftip!="Todos": df = df[df["tipo"]==ftip]
    if fest!="Todos": df = df[df["estilo"]==fest]

    st.dataframe(df.drop(columns=["user_id","photo_path"]), use_container_width=True)

    # Editor/borrador rápido
...
if outfits_df.empty:
    ...
    if sel_o:
        ...
        if oc1.button("Actualizar outfit"):
            db_outfit_update(row["id"], {"name": new_name}); st.success("Outfit actualizado ✅"); st.rerun()
        if oc2.button("Eliminar outfit"):
            db_outfit_delete(row["id"]); st.success("Outfit eliminado ✅"); st.rerun()

# Crear outfit manual
st.markdown("### ✍️ Crear outfit manual")
o_name = st.text_input("Nombre del outfit")
c7, c8 = st.columns(2)
with c7: tmin = st.number_input("Temp. mínima", value=12, step=1)
with c8: tmax = st.number_input("Temp. máxima", value=22, step=1)
...
if eleg>=4 and choices:
    ...
    if neon_mask.any(): st.warning("Has elegido colores tipo neón; no recomendado para un outfit **Formal**.")
...
if st.button("💾 Guardar outfit manual"):
    if not o_name or not item_ids:
        ...
        db_outfit_insert({
            "name": o_name, "temp_min": int(tmin), "temp_max": int(tmax),
            "seasons": seasons, "elegance": int(eleg), "item_ids": item_ids
        })
...
with aa1: atmin = st.number_input("Temp. mínima (auto)", value=12, step=1)
with aa2: atmax = st.number_input("Temp. máxima (auto)", value=22, step=1)
with aa3: aseason = st.selectbox("Estación (auto)", SEASONS, index=1)
...
def filter_for_weather(df, tmin, tmax, eleg):
    ...
    if tmax >= 23:
        ...
        df = df[~(df["categoria"].isin(["Botas"]))]  # menos botas en calor
    if tmax <= 12:
...
def auto_build(df, tmin, tmax, eleg):
    ...
    if tops.empty or bottoms.empty or shoes.empty: return None
    ...
    # Bottom compatible
    bpool = bottoms.copy(); bhex = (bpool["color1_hex"].replace("", pd.NA).fillna(bpool["color2_hex"])).tolist()
    wb = pick_by_color(top_hex, bhex)
    bottom = bpool.iloc[[bhex.index(np.random.choice(wb))]] if wb else bottoms.sample(1)
    # Zapatos por clima
    if atmax>=25 and not shoes[shoes["categoria"]=="Sandalias"].empty: spool = shoes[shoes["categoria"]=="Sandalias"]
    elif atmax<=12 and not shoes[shoes["categoria"]=="Botas"].empty: spool = shoes[shoes["categoria"]=="Botas"]
    else: spool = shoes
...
if "last_suggest" in st.session_state:
    ...
    if st.button("Guardar esta propuesta"):
        if not aname: st.error("Pon un nombre al outfit.")
        else:
            db_outfit_insert({
                "name": aname, "temp_min": int(atmin), "temp_max": int(atmax),
                "seasons": [aseason], "elegance": int(aeleg), "item_ids": ids
            })
